...
class LogicShopData:
    ...
    def encodeShopOffers(self):
        ...
        for x in LogicShopData.offers:
            ...
            elif x['ClaimID'] == "starrroad_superrare" and len(self.player.brawlers_unlocked) < 3:
                self.writeUInt8(True)
            elif x['ClaimID'] == "starrroad_epic" and len(self.player.brawlers_unlocked) < 5:
                self.writeUInt8(True)
            elif x['ClaimID'] == "starrroad_megaepic" and len(self.player.brawlers_unlocked) < 7:
                self.writeUInt8(True)
            elif x['ClaimID'] == "starrroad_legendary" and len(self.player.brawlers_unlocked) < 10:
                self.writeUInt8(True)
            else:
