...
class Helpers:
    ...
    def timerglobal(self, timer_start, timer_end):
        ...
        if not int(datetime.today().strftime('%d')) < timer_start[1] and not int(datetime.today().strftime('%H')) < timer_start[0] and not int(datetime.today().strftime('%m')) < timer_start[2]:
            if int(datetime.today().strftime('%m')) == timer_end[2] or int(datetime.today().strftime('%m')) > timer_end[2]:
                print("1")
                timer = timer_max - int(timer_now)
            elif int(datetime.today().strftime('%m')) < timer_end[2] and int(datetime.today().strftime('%m')) + 1 == timer_end[2]:
                print("2")
                if timer_end[2] == 2: timer = timer_max + (86400 * 31) - int(timer_now)
                if timer_end[2] == 3: timer = timer_max + (86400 * 30) - int(timer_now)
            elif int(datetime.today().strftime('%m')) < timer_end[2] and int(datetime.today().strftime('%m')) + 1 < timer_end[2]:
                print("2")
                if timer_end[2] == 2: timer = timer_max + (86400 * 31) + (86400 * 30) - int(timer_now)
        else:
            timer = -1
        return timer
    ...
    def load_account(self, player_data):
        ...
        self.player.brawler_rare = player_data['BrawlerRare']
        self.player.brawler_superrare = player_data['BrawlerSuperRare']
        self.player.brawler_epic = player_data['BrawlerEpic']
        self.player.brawler_megaepic = player_data['BrawlerMegaEpic']
        self.player.brawler_legendary = player_data['BrawlerLegendary']
        ...
        self.player.maintenance = player_data['Maintenance']
        self.player.maintenance_time = player_data['MaintenanceTime']
