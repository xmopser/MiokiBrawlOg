...
class KeepAliveOkMessage(Writer):
    ...
    def encode(self):
        ...
        if int(datetime.today().strftime('%d')) != self.player.day:
            ...
            if len(sorted(set([20, 11, 47, 5, 28, 110, 131, 235, 266, 56, 61, 140, 243, 229, 70, 145]) - set(self.player.unlocked_skins))) != 0:
                self.player.dailyskins[1] = random.choice(sorted(set([20, 11, 47, 5, 28, 110, 131, 235, 266, 56, 61, 140, 243, 229, 70, 145]) - set(self.player.unlocked_skins)))
                self.db.update_player_account(self.player.token, 'DailySkins',  self.player.dailyskins)
                try:
            ...
            if len(sorted(set([44, 68, 90, 71, 147, 90, 91, 26, 30, 128, 210, 96, 60, 135, 231, 246, 248, 72, 69, 146, 248, 170, 171, 172]) - set(self.player.unlocked_skins))) != 0:
                self.player.dailyskins[2] = random.choice(sorted(set([44, 68, 90, 71, 147, 90, 91, 26, 30, 128, 210, 96, 60, 135, 231, 246, 248, 72, 69, 146, 248, 170, 171, 172]) - set(self.player.unlocked_skins)))
                self.db.update_player_account(self.player.token, 'DailySkins',  self.player.dailyskins)
                try:
            ...
            if len(sorted(set([49, 236, 143]) - set(self.player.unlocked_skins))) != 0:
                self.player.dailyskins[3] = random.choice(sorted(set([49, 236, 143]) - set(self.player.unlocked_skins)))
                self.db.update_player_account(self.player.token, 'DailySkins',  self.player.dailyskins)
                try:
        ...
        if len(sorted(set([20, 11, 47, 5, 28, 110, 131, 235, 266, 56, 61, 140, 243, 229, 70, 145]) - set(self.player.unlocked_skins))) != 0 and self.player.dailyskins[1] == 0:
            self.player.dailyskins[1] = random.choice(sorted(set([20, 11, 47, 5, 28, 110, 131, 235, 266, 56, 61, 140, 243, 229, 70, 145]) - set(self.player.unlocked_skins)))
            self.db.update_player_account(self.player.token, 'DailySkins',  self.player.dailyskins)
            try:
                self.player.claimshop.remove("skin2")
        ...
        if len(sorted(set([44, 68, 90, 71, 147, 90, 91, 26, 30, 128, 210, 96, 60, 135, 231, 246, 248, 72, 69, 146, 248, 170, 171, 172]) - set(self.player.unlocked_skins))) != 0 and self.player.dailyskins[2] == 0:
            self.player.dailyskins[2] = random.choice(sorted(set([44, 68, 90, 71, 147, 90, 91, 26, 30, 128, 210, 96, 60, 135, 231, 246, 248, 72, 69, 146, 248, 170, 171, 172]) - set(self.player.unlocked_skins)))
            self.db.update_player_account(self.player.token, 'DailySkins',  self.player.dailyskins)
            try:
                self.player.claimshop.remove("skin3")
        ...
        if len(sorted(set([49, 236, 143]) - set(self.player.unlocked_skins))) != 0 and self.player.dailyskins[3] == 0:
            self.player.dailyskins[3] = random.choice(sorted(set([49, 236, 143]) - set(self.player.unlocked_skins)))
            self.db.update_player_account(self.player.token, 'DailySkins',  self.player.dailyskins)
            try:
                self.player.claimshop.remove("skin4")
